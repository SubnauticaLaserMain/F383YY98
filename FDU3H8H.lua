...
local function MakeESP(entityModel)
	local o = 0

	while entityModel == nil do
		task.wait(0.5)
		
		if entityModel ~= nil or o > 5000 then
			break
		end
	...
	local U = Instance.new("Highlight", entityModel)
	U.Adornee = entityModel
	U.FillTransparency = 1
	U.Name = 'ESP-Part'



	local function createPlayerBoxWithText(model)
		-- Get the player's character and the model bounding box size
		local character = model
		if not character then
			warn("Character not found for player: " .. model.Name)
			return
		end

		-- Calculate the extents size (bounding box size) of the character
		local modelSize = character:GetExtentsSize()
		local rootPart = character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Torso") -- Use root for central positioning

		local Attempting = 0
		local MaxAttempts = 6000

		if not rootPart then
			warn("Root part not found for player: " .. model.Name)

			while rootPart == nil do
				if Attempting > MaxAttempts then
					warn('Attempts where too many times at Line 462')
					return
				end
				...
				rootPart = character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Torso") -- Use root for central positioning
				task.wait(0.1)
				Attempting += 1
			end
		end
		...
		function calculateProzentFromValueAndMax(value, max)
			if max == 0 then
		...
		-- Create a text label above the box
		local text = Drawing.new("Text")
		text.Text = 'HP: ' .. math.floor(calculateProzentFromValueAndMax(model:FindFirstChildOfClass('Humanoid') and model:FindFirstChildOfClass('Humanoid').Health or 0, model:FindFirstChildOfClass('Humanoid') and model:FindFirstChildOfClass('Humanoid').MaxHealth or 100)) .. '%' -- Display the player's name
		text.Size = 20
		text.Center = true
		text.Outline = true
		text.Color = Color3.new(1, 1, 1) -- White color

		-- Function to update the box and text position based on the model's screen position
		local function updateBox()
			local camera = workspace.CurrentCamera
			local partPos, isVisible = camera:WorldToViewportPoint(rootPart.Position)
			local halfModelSize = modelSize / 2

			-- Calculate corners of the box in 3D world space based on the entire model size
			local topLeftWorld = rootPart.Position + Vector3.new(-halfModelSize.X, halfModelSize.Y, 0)
			local topRightWorld = rootPart.Position + Vector3.new(halfModelSize.X, halfModelSize.Y, 0)
			local bottomLeftWorld = rootPart.Position + Vector3.new(-halfModelSize.X, -halfModelSize.Y, 0)
			local bottomRightWorld = rootPart.Position + Vector3.new(halfModelSize.X, -halfModelSize.Y, 0)
			...
			if isVisible then -- Make sure the part is in front of the camera
				-- Set the text position above the box (above the character)
				text.Position = Vector2.new((topLeft.X + topRight.X) / 2, topLeft.Y - 20)
				text.Visible = true

				text.Text = 'HP: ' .. math.floor(calculateProzentFromValueAndMax(model:FindFirstChildOfClass('Humanoid') and model:FindFirstChildOfClass('Humanoid').Health or 0, model:FindFirstChildOfClass('Humanoid') and model:FindFirstChildOfClass('Humanoid').MaxHealth or 100)) .. '%'
			else
				-- Hide the lines and text if the part is behind the camera
				text.Visible = false
			end
		end

		-- Continuously update the box and text positions in a RenderStepped loop
		local connection = game:GetService("RunService").RenderStepped:Connect(updateBox)

		-- Return a function to remove the box and text
		return function()
	...
	-- Example usage:
	local removeBoxFunction = createPlayerBoxWithText(entityModel)
...
-- MakeESP(game.Players.LocalPlayer.Character)
...
if game.PlaceId == 13864667823 then
	...
	if isAdmin == true then
		...
		local FunStuff = Window:CreateTab('Admin - Fun')


		FunStuff:CreateSection('The House')


		FunStuff:CreateLabel('The Gym')

		FunStuff:CreateButton({
			Name = 'Remove Tredmills',
			Callback = function()
				local Tredmills = game:GetService("Workspace"):FindFirstChild('Tredmills')

				if not Tredmills then return end
		...
		FunStuff:CreateButton({
			Name = 'Remove BenchPresses',
			Callback = function()
				local benchpresses = game:GetService("Workspace"):FindFirstChild('BenchPresses')
				local houseBenchpresses = game:GetService("Workspace"):FindFirstChild('TheHouse')

				print(benchpresses, houseBenchpresses)

				if not benchpresses then return end
				if not houseBenchpresses then return end


				Delete(benchpresses)

				for i, v in pairs(houseBenchpresses:GetChildren()) do
					if v and v.Name == 'Benchpress' then
						Delete(v)
					end
				end
			end
		...
		FunStuff:CreateButton({
			Name = 'Remove Gym Decoration',
			Callback = function()
				local ToolCheck = game:GetService('Workspace'):FindFirstChild('TheHouse')

				if ToolCheck and ToolCheck:FindFirstChild('Tool') then
					for i, v in ToolCheck:GetChildren() do
						if v and (v.Name == 'Tool' and v.ClassName == 'Model') then
							Delete(v)
						end
					end
				end
		...
		FunStuff:CreateButton({
			Name = 'Remove Gym Door',
			Callback = function()
				local ReplicatedStorageDoor = game:GetService('ReplicatedStorage'):WaitForChild('Doors2', 20)
				local WorkspaceDoor = game:GetService('Workspace'):WaitForChild('Doors2', 20)

				if ReplicatedStorageDoor then
					local Door = ReplicatedStorageDoor:FindFirstChild('DoorGym')
				...
				if WorkspaceDoor then
					local Door = WorkspaceDoor:FindFirstChild('DoorGym')
		...
		FunStuff:CreateSection(' ')
		FunStuff:CreateLabel('Shop')

		FunStuff:CreateButton({
			Name = 'Delete Food - Vending Machine',
			Callback = function()
				local VendingMachines = game:GetService('Workspace'):FindFirstChild('VendingMachines')


				if VendingMachines then
					local food = VendingMachines:FindFirstChild('Food')
		...
		FunStuff:CreateButton({
			Name = 'Delete Weapons - Vending Machine',
			Callback = function()
				local VendingMachines = game:GetService('Workspace'):FindFirstChild('VendingMachines')

				
				if VendingMachines then
					local weapons = VendingMachines:FindFirstChild('Weapons')
		...
		FunStuff:CreateButton({
			Name = 'Delete BloxyCola - Vending Machine',
			Callback = function()
				local VendingMachines = game:GetService('Workspace'):FindFirstChild('VendingMachines')

				if VendingMachines then
					local Drinks = VendingMachines:FindFirstChild('Drinks')
		...
		FunStuff:CreateButton({
			Name = 'Delete Drawers',
			Callback = function()
				local Dressers = game:GetService('Workspace'):FindFirstChild('Dressers')

				if Dressers then
					local DresserShop = Dressers:FindFirstChild('DresserShop')
		...
		FunStuff:CreateButton({
			Name = 'Remove Money Table',
			Callback = function()
				local TableSetMoney = game:GetService('Workspace'):FindFirstChild('TableSetMONEY')

				if TableSetMoney then
					Delete(TableSetMoney)
				end
			end
		})

		FunStuff:CreateButton({
			Name = 'Delete Weapon Poster',
			Callback = function()
				local TheHouse = game:GetService('Workspace'):FindFirstChild('TheHouse')

				if TheHouse then
					local PosterWeapon = TheHouse:FindFirstChild('PosterWeapon')

					if PosterWeapon then
						Delete(PosterWeapon)
					end
				end
			end
		...
		FunStuff:CreateButton({
			Name = 'Remove Shop Door',
			Callback = function()
				local Doors1 = game:GetService('Workspace'):FindFirstChild('Doors1')

				if Doors1 then
					local ShopDoor = Doors1:FindFirstChild('DoorShop')
		...
		FunStuff:CreateLabel('The Arena - Fight')


		FunStuff:CreateButton({
			Name = 'Delete BadGuys',
			Callback = function()
				local folder = game:GetService('Workspace'):FindFirstChild('BadGuys')
		...
		-- Will make it so, no more badguys will spawn.
		FunStuff:CreateParagraph({
			Title = 'NOTE',
			Content = `'Delete Badguys' will result in the Enemies in the Fighting Arena not spawning anymore.`
		})


		FunStuff:CreateButton({
			Name = 'Delete MetalDoor',
			Callback = function()
				local TheHouse = game:GetService('Workspace'):FindFirstChild('TheHouse')

				if TheHouse then
					local MetalDoor = TheHouse:FindFirstChild('MetalBitBreakRoom')
		...
		FunStuff:CreateButton({
			Name = 'Delete Door - 4 Strenght Door',
			Callback = function()
				local TheHouse = game:GetService('Workspace'):FindFirstChild('TheHouse')

				if TheHouse then
					local Door = TheHouse:FindFirstChild('Door')
		...
		FunStuff:CreateButton({
			...
			Callback = function()
				...
				if TheHouse then
					local BreakRoomCamera = TheHouse:FindFirstChild('BreakRoomCamera')

					if BreakRoomCamera then
						Delete(BreakRoomCamera)
					end
				end
			end
		})
		...
		FunStuff:CreateButton({
			...
			Callback = function()
				...
				if TheHouse then
					local BreakRoom = TheHouse:FindFirstChild('BreakRoom')

					if BreakRoom then
						local TVSetup = BreakRoom:FindFirstChild('TVSetup')

						if TVSetup then
							local TV = TVSetup:FindFirstChild('TV')
			...
			Callback = function()
				...
				if TheHouse then
					...
					if BreakRoom then
						...
						if TVSetup then
							local TV = TVSetup:FindFirstChild('Shelf')
		...
		FunStuff:CreateButton({
			Name = 'Delete BreakRoomEnemies',
			Callback = function()
				local TheHouse = game:GetService('Workspace'):FindFirstChild('TheHouse')

				if TheHouse then
					local BreakRoomModels = TheHouse:FindFirstChild('BreakRoomModels')

					if BreakRoomModels then
						Delete(BreakRoomModels)
					end
				end
			end
		})
		...
		FunStuff:CreateButton({
			Name = 'Delete Armor - Vending Machine',
			Callback = function()
				local VendingMachines = game:GetService('Workspace'):FindFirstChild('VendingMachines')

				if VendingMachines then
					local Armor = VendingMachines:FindFirstChild('Armor')
		...
		FunStuff:CreateButton({
			...
			Callback = function()
				...
				if TheHouse then
					...
					if BreakRoom then
						local Table = BreakRoom:FindFirstChild('Table')
		...
		FunStuff:CreateButton({
			...
			Callback = function()
				...
				if TheHouse then
					for i, v in TheHouse:GetChildren() do
						print(tostring(i), tostring(v))
						if v:FindFirstChild('Model') and v.Name == 'Shelf' and #v:FindFirstChild('Model'):GetChildren() == 4 then
							print(tostring(v) .. ' Shelf was found!')
							Delete(v)
							break
						end
					end
		...
		FunStuff:CreateButton({
			Name = 'Delete Box',
			Callback = function()
				local Boxes = game:GetService('Workspace'):FindFirstChild('Boxes')

				if Boxes then
					local box = Boxes:GetChildren()



					for i, v in pairs(box) do
						if v and v:FindFirstChild('PlaceHere') and v:FindFirstChild('PlaceHere').ClassName == 'Part' and v:FindFirstChild('PlaceHere').Position == Vector3.new(-207.03416442871094, 64.92413330078125, -720.1553955078125) then
							if #box == 1 then
								Delete(Boxes)
								return
							end
		...
		FunStuff:CreateButton({
			...
			Callback = function()
				...
				if TheHouse then
					local Glass = TheHouse:FindFirstChild('Glass')
		...
		AdminPlayers:CreateSection('Player Stuff')
		
		local RemovePlayerInventory = AdminPlayers:CreateButton({
			Name = 'Remove Players Inventory',
			Callback = function()
				local a, b = pcall(function()
					if not game:GetService('Players'):FindFirstChild(sel) then
						Rayfield:Notify({
					...
					Rayfield:Notify({
						Title = 'Confirm',
						Content = 'Are you sure you want to Clear ' .. tostring(sel) .. ' Inventory?',
						Duration = 60,
						Image = 4483362458,
						Actions = {
							Confirm = {
								Name = 'Yes',
								Callback = function()
									for i, v in game:GetService('Players'):FindFirstChild(sel):WaitForChild('Backpack'):GetChildren() do
										Delete(v)
									end
								end
							},
		...
		AdminPlayers:CreateButton({
			Name = 'Remove Holding Item',
			Callback = function()
				local a, b = pcall(function()
					if not game:GetService('Players'):FindFirstChild(sel) then
						Rayfield:Notify({
					...
					Rayfield:Notify({
						Title = 'Confirm',
						Content = 'Are you sure you want to Clear ' .. tostring(sel) .. ' Holding Item?',
						Duration = 60,
						Image = 4483362458,
						Actions = {
							Confirm = {
								Name = 'Yes',
								Callback = function()
									for i, v in game:GetService('Players'):FindFirstChild(sel).Character:GetChildren() do
		...
		AdminPlayers:CreateSection('Other')
		...
		AdminPlayers:CreateButton({
			...
			Callback = function()
				local a, b = pcall(function()
					Rayfield:Notify({
						Title = 'Confirm',
						Content = 'Are you sure you want to Kick Everyone?',
						Duration = 60,
						Image = 4483362458,
						Actions = {
							Confirm = {
		...
		AdminPlayers:CreateButton({
			...
			Callback = function()
				local a, b = pcall(function()
					Rayfield:Notify({
						Title = 'Confirm',
						Content = 'Are you sure you want to Clear Everyone?',
						Duration = 60,
						Image = 4483362458,
						Actions = {
							Confirm = {
								...
								Callback = function()
									for i, v in game:GetService('Players'):GetPlayers() do
										if v ~= game:GetService('Players').LocalPlayer then
											if v:WaitForChild('Backpack') then
												for a, b in v.Backpack:GetChildren() do
													Delete(b)
												end
											end
										end
	...
	local TeleportationFolderStorage = Instance.new('Folder', placement)
	TeleportationFolderStorage.Name = 'TeleportationFolderStorage'

	local function CVector3(Name, Vec)
		local CFrameStorage = Instance.new('Vector3Value', TeleportationFolderStorage)
		CFrameStorage.Name = Name
		CFrameStorage.Value = Vec
	end


	CVector3('Spawn', Vector3.new(83.55531311035156, 28.098604202270508, -530.071044921875))
	CVector3('Bunker', Vector3.new(-241.52024841308594, 29.24827766418457, -790.492919921875))
	CVector3('Arena - Fight', Vector3.new(-262.471435546875, 61.362545013427734, -725.4645385742188))
	...
	local ESPs = {}
	local ESP_BadGuys_Added_Signal = nil
	local ESP_BadGuys_Removed_Signal = nil
	...
	RenderTab:CreateToggle({
		...
		Callback = function(val)
			if val == true then
				if ESP_BadGuys_Added_Signal == nil then
					if ESP_BadGuys_Removed_Signal then
						ESP_BadGuys_Removed_Signal:Disconnect()
						ESP_BadGuys_Removed_Signal = nil
					end


					local BadGuysFolder = game:GetService('Workspace'):FindFirstChild('BadGuys')


					if BadGuysFolder then
						for i, v in BadGuysFolder:GetChildren() do
							ESPs[v] = MakeESP(v)
						end

						ESP_BadGuys_Added_Signal = BadGuysFolder.ChildAdded:Connect(function(e)
							ESPs[e] = MakeESP(e)
						end)


						ESP_BadGuys_Removed_Signal = BadGuysFolder.ChildRemoved:Connect(function(e)
							ESPs[e]()
						end)
					end
				end
			else
				if ESP_BadGuys_Added_Signal then
					ESP_BadGuys_Added_Signal:Disconnect()
					ESP_BadGuys_Added_Signal = nil
				end
			end
		end
	})
	...
	function GetItemAndFireRemote(r, a, g, r2)
		if typeof(r) == 'Instance' and a and r2 then
			if r.ClassName == 'RemoteEvent' then
				r:FireServer(table.unpack(a))
				game:GetService('Players').LocalPlayer:WaitForChild('Backpack').ChildAdded:Wait()
				task.wait(0.5)
				Equip(game:GetService('Players').LocalPlayer:WaitForChild('Backpack'):FindFirstChild(g))
				task.wait(0.25)
				r2:FireServer()

				do
					if game:GetService('Players').LocalPlayer:WaitForChild('Backpack'):FindFirstChild(g) then
						game:GetService('Players').LocalPlayer:WaitForChild('Backpack'):FindFirstChild(g):Destroy()
						return
					end

					if (game:GetService('Players').LocalPlayer.Character or game:GetService('Players').LocalPlayer.CharacterAdded:Wait()):FindFirstChild(g) then
						(game:GetService('Players').LocalPlayer.Character or game:GetService('Players').LocalPlayer.CharacterAdded:Wait()):FindFirstChild(g):Destroy()
						return
					end
				end
			end
	...
	local NPCs = Window:CreateTab('NPC')

	NPCs:CreateButton({
		Name = 'Recruit UnclePete',
		Callback = function()
			GetItemAndFireRemote(game:GetService('ReplicatedStorage'):WaitForChild('Events', 20).GiveTool, {[1] = 'Key'}, 'Key', game:GetService('ReplicatedStorage'):WaitForChild('Events', 20).KeyEvent)
		end
	})


	local Other = Window:CreteTab('Other')
